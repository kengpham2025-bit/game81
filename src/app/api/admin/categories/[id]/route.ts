...
export async function PUT(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const cat = await categoryDB.update({ where: { id }, data: body });
    return NextResponse.json(cat);
  } catch (e) {
    if ((e as Error).message === 'Không được phép') return NextResponse.json({ error: 'Không được phép' }, { status: 401 });
    if ((e as Error).message === 'Category not found') return NextResponse.json({ error: 'Không tìm thấy' }, { status: 404 });
...
export async function DELETE(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    await requireAdmin();
    const { id } = await params;
    await categoryDB.delete({ where: { id } });
